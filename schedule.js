...
(function() {
  ...
  /**
  * Schedules human work (tasks) as part of a project. Schedule
  ...
  */
  var Schedule = function(resolution, startTime, useLocalTime) {
    ...
    var scheduleTask = function(task, time, completeCache, scheduledTasks) {
      ...
      // make sure that assignedTo is available and allocate their time
      if(assignedTo !== undefined) {
        var p = people[assignedTo];
        if(p.rangeStartTime <= time) {
          sLength = Math.min(sLength, (p.rangeEndTime - time) / MIN);
          p.rangeStartTime = time + (sLength * MIN);
          p.rangeLength = (p.rangeEndTime - p.rangeStartTime) / MIN;
        }
        else {
    ...
    var updateRange = function(s, time) {
      ...
      var start, end;
      if((start = l.getNext(s, tDate))) {
        s.rangeStartTime = start.getTime();
        end = l.getNextInvalid(s, start);
        s.rangeEndTime = end ? end.getTime() : MAX;
        s.rangeLength = (s.rangeEndTime - s.rangeStartTime) / MIN;
      }
      else {
        s.rangeStartTime = s.rangeEndTime = undefined;
        s.rangeLength = 0;
      }
    };
    ...
    var scheduleTasks = function(tasks) {
      ...
      while(time) {
        ...
        for(var i = 0; i < len; i++) {
          var tNext = scheduleTask(tasks[i], time, completeCache, scheduledTasks);
          if(tNext && (!next || tNext < next)) next = tNext;
        }
    ...
    return {
      ...
      *
      * Returns a set of schedule items that defines the start date, end date,
      * and length of the scheduled task.
      *
